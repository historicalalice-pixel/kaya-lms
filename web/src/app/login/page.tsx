...
export default function LoginPage() {
  ...
  // ── Визначити куди редіректити по ролі ────────────────────────────────────
  ...
  const handleOAuthLogin = async (provider: "google") => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${redirectBase}/auth/callback?next=/auth/role-redirect`,
        },
      });
        ...
        <div className="w-full max-w-[500px] md:translate-y-4">
          ...
          <div className="rounded-[20px] border border-[rgba(201,169,110,0.12)] bg-[rgba(10,10,14,0.3)] backdrop-blur-[6px] px-5 py-7 md:px-7 md:py-9 shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_14px_40px_rgba(0,0,0,0.24)]">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="email" className="block font-sans text-[0.94rem] text-[rgba(245,239,230,0.78)] mb-2">
                  Email
                </label>
                <input
                  id="email"
              ...
              <div>
                <label htmlFor="password" className="block font-sans text-[0.94rem] text-[rgba(245,239,230,0.78)] mb-2">
                  Пароль
                </label>
                <input
                  id="password"
              ...
              {errorMessage && (
                <p className="font-sans text-[0.92rem] text-red-400">{errorMessage}</p>
              )}
