...
const TOPICS = [
  {
    ...
    territory: {
      ...
      geometry: {
        ...
        coordinates: [[
          // Рухаємось за годинниковою стрілкою
          // Північ-захід: Псков/Новгород
          [28.0, 57.5],
          // Північ-схід: Ростов Великий
          [39.5, 57.0],
          // Схід: Рязань
          [40.5, 54.5],
          // Схід-південь: Чернігів
          [37.5, 51.5],
          // Південь-схід: Тмутаракань (Крим/Азов)
          [36.5, 47.5],
          // Південь: Херсонес
          [33.5, 45.5],
          // Південь-захід: Олешшя/Дунай
          [30.0, 46.5],
          // Захід: Галич
          [24.0, 49.0],
          // Захід-північ: Берестя (Брест)
          [24.0, 52.0],
          // Північ-захід: Полоцьк
          [28.5, 55.5],
  ...
  },
  {
    ...
    territory: {
      ...
      geometry: {
        ...
        coordinates: [[
          ...
          // Північ: Стародуб, Чернігів
          [32.0, 53.0], [33.5, 52.5], [35.0, 52.0],
          // Схід: Харків, Полтава
          [36.5, 51.0], [37.2, 50.0], [36.8, 49.0],
          // Південь: Запоріжжя (Вольності)
          [35.5, 47.5], [34.0, 47.0], [33.0, 46.5],
          // Захід: по Дніпру
          [31.5, 46.5], [30.5, 47.5],
          // Поділля, Брацлав
          [28.5, 48.5], [27.5, 49.5],
          // Захід Волині
          [26.5, 51.0], [28.0, 52.0], [30.0, 52.8],
          [32.0, 53.0]
        ]]
      }
    },
    events: [
      ...
      { id: 3, year: 1654, title: "Переяславська рада",       coords: [31.45, 50.08] as [number, number], desc: "Рада у Переяславі. Гетьман Хмельницький укладає союз з Московським царством." },
  ...
  },
  {
    ...
    territory: {
      ...
      geometry: {
        type: "Polygon" as const,
        coordinates: [[
          // УНР + ЗУНР після Акту Злуки 1919
          // Північ: Волинь, Чернігів
          [24.0, 51.5], [27.5, 52.0], [30.5, 52.5], [33.5, 52.0], [35.5, 51.5],
          // Схід: Харків, Луганськ
          [36.5, 50.5], [38.5, 49.5], [38.0, 48.0],
          // Південь: Херсон, Одеса
          [35.0, 46.5], [33.5, 46.0], [32.0, 46.5], [31.0, 46.2],
          [29.5, 46.0], [28.5, 46.2],
          // Захід: ЗУНР (Галичина + Буковина)
          [22.5, 48.5], [22.0, 49.0], [22.5, 50.5],
          [23.0, 51.0], [24.0, 51.5]
        ]]
      }
    },
    events: [
      ...
      { id: 2, year: 1918, title: "IV Універсал. Незалежність", coords: [30.52, 50.40] as [number, number], desc: "22 січня 1918 — УНР проголошує повну незалежність від Росії. Перша незалежна українська держава." },
      ...
      { id: 4, year: 1919, title: "Акт Злуки",                  coords: [30.52, 50.48] as [number, number], desc: "22 січня 1919 у Києві — урочисте об'єднання УНР і ЗУНР в єдину соборну українську державу." },
    ],
    figures: [
...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    activeTopic.events.forEach((ev, index) => {
      const el = document.createElement("div");
      el.className = "kaya-marker";

      // Зміщення для маркерів що стоять близько (Київська Русь — 882, 988, 1037 всі в Києві)
      ...
      new mapboxgl.Marker({ element: el, anchor: "center", offset: offset }).setLngLat(ev.coords).addTo(map);
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
